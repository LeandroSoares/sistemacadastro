<div class="p-6 bg-white rounded-lg shadow-md">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 text-right space-x-2">
                            <button wire:click="editAmaci({{ $amaci->id }})"                                class="text-blue-600 hover:text-blue-900">
                                Editar
                            </button>
                            <button wire:click="deleteAmaci({{ $amaci->id }})"                                class="text-red-600 hover:text-red-900"                                onclick="return confirm('Tem certeza que deseja excluir este amaci?')">
                                Excluir
                            </button>
                        </td>
                    </tr>
    ...
    <form wire:submit="save" class="space-y-6 mt-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
                <label class="block text-sm font-medium text-gray-700">Tipo de Amaci
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700">Observações
                    <input type="text"                    wire:model="observations"                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                </label>
                @error('observations')
                    <span class="text-red-500 text-xs">{{ $message }}</span>
                @enderror
        ...
        <div class="flex justify-end mt-6 space-x-2">
            @if($editing)
                <button type="button"                    wire:click="cancelEdit"                    class="px-4 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-700">
                    Cancelar
                </button>
            @endif
